...
// === Referencias de colecciones / docs ===
...
export const getProjects = async (): Promise<Project[]> => {
  const snap = await getDocs(
    query(projectsCol, orderBy('created_at', 'desc')),
  );
  return snap.docs.map((d) => ({
    id: d.id,
...
// Versión mínima: asume que los docs ya tienen forma MonthlyReport.
// Si está vacío, Reports mezclará igual con LEGACY_HISTORY y no se rompe nada.
...
// Implementación sencilla de cierre: agrega un reporte agregando gastos del periodo.
