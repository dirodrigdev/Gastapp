...
export const subscribeToExpenses = (callback: (data: MonthlyExpense[]) => void) => {
  console.log("📡 Intentando conectar a gastos en la nube...");
  const q = query(collection(db, EXPENSES_COL));
  
  return onSnapshot(q, (snapshot) => {
    console.log(`✅ Conexión exitosa. Se recibieron ${snapshot.docs.length} gastos.`);
    const data = snapshot.docs.map(d => ({ id: d.id, ...d.data() } as MonthlyExpense));
    callback(data);
  }, (error: any) => {
    console.error("❌ ERROR CRÍTICO en conexión a Firebase:", error.code, error.message);
    if (error.code === 'permission-denied') {
        alert("⚠️ ERROR: Firebase denegó el permiso. Revisa la pestaña 'Reglas' en la consola de Firebase.");
    }
  });
};
...
export const addMonthlyExpense = async (expense: Omit<MonthlyExpense, 'id'>): Promise<void> => {
  try { 
      console.log("📤 Enviando gasto a la nube...", expense);
      await addDoc(collection(db, EXPENSES_COL), expense); 
      console.log("✅ Gasto guardado en la nube correctamente.");
  } catch (e: any) { 
...
export const getCategories = async (): Promise<Category[]> => {
  try {
    ...
    if (cats.length === 0) {
        // CORRECCIÓN: Usamos setDoc para asegurar que el ID de la categoría sea el ID del documento
        console.log("⚠️ Nube vacía. Subiendo categorías por defecto...");
        for (const c of DEFAULT_CATEGORIES) {
             // Subimos el objeto completo, pero usamos c.id para nombrar el documento
             await setDoc(doc(db, CATEGORIES_COL, c.id), c);
        }
        // Leemos de nuevo para retornar los objetos recién creados con sus IDs correctos
        const snapshotAfterInit = await getDocs(collection(db, CATEGORIES_COL));
        return snapshotAfterInit.docs.map(d => ({ id: d.id, ...d.data() } as Category));
    }
    return cats;
  } catch (error: any) { 
...
export const saveCategory = async (category: Category): Promise<void> => {
  try {
    ...
    if (id) {
       // Si tiene ID, actualiza el documento con ese ID
       await setDoc(doc(db, CATEGORIES_COL, id), data, { merge: true });
    } else {
       // Si no tiene ID (es nuevo), Firestore lo crea
