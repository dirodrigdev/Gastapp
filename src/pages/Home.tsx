...
export const Home = () => {
  ...
  useEffect(() => {
      ...
      expenses.forEach(e => { usageCount[e.categoria] = (usageCount[e.categoria] || 0) + 1; });
  ...
  return (
    <div className="p-4 space-y-5 pb-24">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
            <div className="flex gap-2 items-center">
                <div className="flex-1 relative">
                    ...
                    <Input id="amount-input" type="text" inputMode="decimal" aria-label="Monto del gasto" value={amount} onChange={(e) => setAmount(e.target.value)} onBlur={handleAmountBlur} placeholder="0,00" className="pl-8 py-2 text-2xl font-bold border-none bg-slate-50 focus:ring-0 rounded-lg text-slate-800 placeholder:text-slate-200" />
                </div>
                <div className="relative">
                    <button type="button" aria-label="Cambiar fecha del gasto" className="h-12 w-12 bg-slate-50 rounded-lg flex flex-col items-center justify-center text-blue-600 border border-slate-100 relative overflow-hidden">
                        ...
                        <input type="date" aria-label="Selector de fecha" className="absolute inset-0 opacity-0 cursor-pointer" value={expenseDate} min={format(activeStartDate, 'yyyy-MM-dd')} onChange={(e) => setExpenseDate(e.target.value)} />
                    </button>
                </div>
            </div>
            ...
            <div className="flex gap-2">
                <Input id="description-input" aria-label="Descripción del gasto (opcional)" value={description} onChange={e => setDescription(e.target.value)} placeholder="Descripción (Opcional)" className="py-2 text-sm bg-slate-50 border-none" />
                <Button type="submit" disabled={loading} aria-label="Agregar Gasto" className="aspect-square p-0 w-10 h-10 rounded-xl bg-blue-600 text-white shrink-0"><Plus size={20} aria-hidden="true" /></Button>
            </div>
        </form>
      </Card>
      ...
      <Card className="bg-slate-900 text-white p-5 border-none shadow-xl relative overflow-hidden">
         <div className="flex justify-between items-start">
             ...
             <div className="relative h-16 w-16" aria-hidden="true">
                 <svg className="w-full h-full" viewBox="0 0 64 64">
                     ...
                     <circle cx="32" cy="32" r={radius} stroke="currentColor" strokeWidth="4" fill="transparent" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} className={donutColor} strokeLinecap="round" transform="rotate(-90 32 32)" />
                     <text x="32" y="32" className="fill-white text-[10px] font-bold" textAnchor="middle" dominantBaseline="central">{totalPercentReal.toFixed(0)}%</text>
                 </svg>
             </div>
         </div>
         ...
         <button onClick={() => setShowBudgetDetails(!showBudgetDetails)} aria-expanded={showBudgetDetails} aria-label="Ver detalles de presupuestos" className="w-full mt-3 flex items-center justify-center pt-2 text-slate-500 hover:text-white transition-colors"><ChevronUp size={16} aria-hidden="true" /></button>
      </Card>
                  ...
                  return (
                      <div key={cat.id} className="bg-white p-3 rounded-xl border border-slate-100 flex items-center gap-3">
                          ...
                          <div className="flex-1">
                              <div className="flex justify-between text-xs mb-1"><span className="font-bold text-slate-700">{cat.nombre}</span><span className="text-slate-500">{Math.round(cat.percent)}%</span></div>
                              <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden" role="progressbar" aria-valuenow={Math.min(100, cat.percent)} aria-valuemin={0} aria-valuemax={100} aria-label={`Progreso de ${cat.nombre}`}><div className={`h-full rounded-full ${colorClass}`} style={{ width: `${Math.min(100, cat.percent)}%` }}></div></div>
