...
export const Budgets = () => {
  ...
  const loadData = async () => {
    // 1) Cargamos categorías, config y reportes en paralelo
    const [catsFromDb, config, reports] = await Promise.all([
      getCategories(),
      getClosingConfig(),
      getMonthlyReports(),
    ]);

    let effectiveCats = catsFromDb;

    // 2) Si Firestore está vacío, intentamos migrar categorías legacy desde localStorage
    if (catsFromDb.length === 0) {
      ...
      if (legacy) {
        try {
          ...
          // Normalizamos al tipo Category actual
          const mapped: Category[] = parsed.map((raw, index) => {
            return {
              id: raw.id || crypto.randomUUID(),
              nombre: raw.nombre || raw.name || `Categoría ${index + 1}`,
              presupuestoMensual:
                typeof raw.presupuestoMensual === 'number'
                  ? raw.presupuestoMensual
              ...
              activa:
                typeof raw.activa === 'boolean'
                  ? raw.activa
                  : raw.active ?? true,
              icono: raw.icono || raw.icon || 'General',
            };
          });

          effectiveCats = mapped;
          setCategories(mapped);

          // Sube en segundo plano a Firestore
          mapped.forEach((cat) => {
          ...
          // Si falla, al menos dejamos las categorías desde Firestore (vacías)
          setCategories(catsFromDb);
        }
      } else {
        // No hay legacy -> usamos lo que venga de Firestore (vacío)
        setCategories(catsFromDb);
      }
    } else {
      // Caso normal: ya hay categorías en Firestore
      effectiveCats = catsFromDb;
      setCategories(catsFromDb);
    }

    // --- CÁLCULO DE PERIODO ACTIVO (misma lógica que Home/Settings) ---
  ...
  return (
    <div className="p-4 space-y-6 pb-24">
      ...
      <Card className="bg-blue-600 text-white p-5 border-none shadow-blue-500/20">
        <h3 className="text-blue-100 text-xs font-bold uppercase tracking-wider mb-1">
          Presupuesto Mensual
        </h3>
      ...
      <div className="flex justify-between items-center mt-6 mb-2">
        ...
        <Button size="sm" onClick={() => handleOpenModal()} className="h-9">
          <Plus size={16} className="mr-1" aria-hidden="true" /> Nueva
        </Button>
      </div>
          ...
          >
            <div>
              ...
              <p className="text-sm text-gray-500">
                € {formatLocaleNumber(cat.presupuestoMensual)}
              </p>
            </div>
        ...
        >
          <Card className="w-full max-w-sm p-6 space-y-4 shadow-xl max-h-[90vh] overflow-y-auto no-scrollbar">
            <div className="flex justify-between items-center">
              <h3 id="cat-modal-title" className="text-lg font-bold">
                {editingCat ? 'Editar' : 'Nueva'}
              </h3>
              <button
            ...
            <div>
              <label
                htmlFor="cat-name"
                className="text-xs font-semibold text-gray-500 ml-1"
              >
                Nombre
              </label>
              <Input
            ...
            <div>
              ...
              <Input
                ...
                onBlur={() =>
                  setBudgetStr(
                    formatLocaleNumber(parseLocaleNumber(budgetStr)),
                  )
                }
              />
            </div>
            ...
            <div>
              <span
                id="icon-label"
                className="text-xs font-semibold text-gray-500 ml-1 mb-2 block"
              >
                Icono
              </span>
              <div
