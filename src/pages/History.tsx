...
export const History = () => {
  ...
  // Suscripción a categorías (una sola vez)
  useEffect(() => {
  ...
  const handleCopyAll = async () => {
    ...
    const textData = expenses
      .map(
        (e) =>
          `${format(new Date(e.fecha), 'dd/MM')};${e.descripcion};${e.categoria};-${formatWithDecimals(
            e.monto,
          )}`,
      )
      .join('\n');
  ...
  return (
    <div className="p-4 space-y-4 pb-24">
      ...
      <div className="space-y-2" role="list">
        ...
        {expenses.length === 0 && (
          <p className="text-center text-gray-400 mt-10">Sin movimientos activos</p>
        )}
      </div>
